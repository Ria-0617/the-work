...
namespace MyFanc {
	...
	inline bool BoxCollider(ci::Vec3f pos, float scale, ci::Vec3f pos2, float scale2) {
		return pos.distance(pos2) > scale + scale2 ? true : false;
	}

	inline float EaseInOutBack(float t, float max, float min, float s)
	{
		max -= min;
		s *= 1.525f;
		if (t / 2 < 1)
